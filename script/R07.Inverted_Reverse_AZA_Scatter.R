...
#library(mining)
cov="2.5"
map = read.table(paste(inpDir,"/Scatter_Inverted_Repeats_Reverse_AZA_ratio.txt",sep=""),header=T)
...
NTnonzero<-dataInv[(dataInv[,1]>0.2)&(dataInv[,5]>0.2),]
...
png(paste(outDir,"/Inverted_Repeats_Reverse_AZA_Scatter-",id,".png",sep=""),width=1500,height=1500,bg="white",res=200)
...
r<-cor.test(data[,1], data[,2],method = c("pearson"))
print ("correlation")
