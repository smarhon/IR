...
library(plotrix)
cov="2.5"
map = read.table(paste(inpDir,"/Inverted_Repeats_Reverse_AZA_ratio_Histogram.txt",sep=""),header=T)


dataInv<-as.matrix(map)

map = read.table(paste(inpDir,"/nonInverted_Repeats_Reverse_AZA_ratio_Histogram.txt",sep=""),header=T)
...
png(paste(outDir,"/Inverted_Repeats_Reverse_AZA_ratio_Histogram-",id,".png",sep=""),width=1500,height=1500,bg="white",res=200)
...
to=maxy-200
from=600
plot(datanonInv[,1],datanonInv[,i],lwd=3,xticlab=NA,yticlab=NA,type="h",ylim=axlimy,cex=1.5,border=mycolblue,main=lmain[i-1],font=2,main.font=2,
		ylab="Counts",xlab=xlabels[i-1],col=mycolblue,cex.main=2,cex.lab=1.5,cex.axis=1.5,axes=F,font.lab=2)
...
plot(dataInv[,1],dataInv[,i],lwd=3,xticlab=NA, yticlab=NA,type="h",ylim=axlimy,col=mycolRed,ylab="",xlab="",cex.lab=1.5,cex.axis=1.3,axes=F,font=2,main.font=2)
...
#legend("top", rnames,ncol=5,bty="n", cex=1.3, fill=cols,pt.cex=12)



#axis(1, at=xbk,labels=xbklab,cex.axis=cexaxis)

#axis(2,cex.axis=cexaxis)


#axis(1,names.arg=xbklab,cex.axis=cexaxis)
